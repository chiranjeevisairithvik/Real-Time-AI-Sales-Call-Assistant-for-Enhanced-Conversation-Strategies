...
laptop_data = pd.read_csv("D:\infosys_project1\infosys_project\laptop_dataset_updated.csv")  # Path to the dataset
...
def main():
    st.set_page_config(page_title="AI-Based Assistant")
    
    
    st.markdown('<div class="title">AI-based Laptop Recommendation </div>', unsafe_allow_html=True)

    credentials_path = "D:\infosys_project1\infosys_project\credentials2.json"
    spreadsheet_id = "1BQRX513_GAiLKokObbJoA30foTKzLC_BFS48LD5MMTM"
    customer_history_path = "D:\infosys_project1\infosys_project\dataset_preparation - Sheet1.csv"
    ...
    conversation = []
    focused_laptops = []
    record_button = st.button("Start", key="record_button")
    is_recorded = False
    while True:
        if record_button:
            st.write("Listening...")
            ...
            if text:
                ...
                if "stop" in text.lower():
                    st.write("stopping the conversation as per the buyer's request.")
